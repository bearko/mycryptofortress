import type { HeroDef, TilePos } from "./types";

/**
 * SPEC-003 §5.2: Common レアリティのヒーロー 8 体（mycryptoheroes/Data/Heroes/heroes.json
 * から ID 1001-1010 のうち、職業 8 種を網羅できる組み合わせを採用）。
 *
 * ステータスは `max_level_stats` をそのまま採用（HP / PHY / INT / AGI）。
 * `attackType` / `class` / `cost` / `attackPattern` はゲームバランス用の派生値で、
 * Passive スキル要約と Arknights 流の役割対応から決定している（SPEC-003 表参照）。
 *
 * SPEC-009 §5.4: phyDef / intDef を職業ベースで初期値設定。MCH 公式マスタは攻撃力
 * のみ持つため、設計ドキュメントの想定（[1lRHoKJ5...](https://docs.google.com/spreadsheets/d/1lRHoKJ5AFGh6dCvKJ41dPwnKuJaVmD8Bm7hlSIXe3ow/edit)）を
 * 参考に職業の役割で振り分けている暫定値:
 *
 *   重装 (defender):  phyDef 60 / intDef 30 — 物理タンク
 *   前衛 (guard):     phyDef 35 / intDef 25 — バランス近接
 *   先鋒 (vanguard):  phyDef 25 / intDef 20 — 軽装速攻
 *   特殊 (specialist):phyDef 30 / intDef 30 — 万能
 *   狙撃 (sniper):    phyDef 15 / intDef 10 — 紙装甲
 *   術師 (caster):    phyDef 10 / intDef 35 — INT 寄り
 *   医療 (medic):     phyDef 20 / intDef 30 — 中盤の支援
 *   補助 (supporter): phyDef 15 / intDef 40 — INT に厚い
 */
const HERO_IMAGE_BASE =
  "https://raw.githubusercontent.com/bearko/mycryptoheroes/main/Image/Heroes";

/** 4 マス直線（遠距離） */
const PATTERN_LINE_4: TilePos[] = [
  { col: 1, row: 0 },
  { col: 2, row: 0 },
  { col: 3, row: 0 },
  { col: 4, row: 0 },
];

/** 3 マス直線（中距離） */
const PATTERN_LINE_3: TilePos[] = [
  { col: 1, row: 0 },
  { col: 2, row: 0 },
  { col: 3, row: 0 },
];

/** 1 マス前 縦 3（前衛 / 重装の近接） */
const PATTERN_NEAR_3: TilePos[] = [
  { col: 1, row: -1 },
  { col: 1, row: 0 },
  { col: 1, row: 1 },
];

/** 縦 3 + 中央 1 マス前（前衛拡張） */
const PATTERN_NEAR_FAN: TilePos[] = [
  { col: 1, row: -1 },
  { col: 1, row: 0 },
  { col: 1, row: 1 },
  { col: 2, row: 0 },
];

/** 自身の周囲 8 タイル（補助 / 医療の支援範囲） */
const PATTERN_AROUND: TilePos[] = [
  { col: -1, row: -1 },
  { col: 0, row: -1 },
  { col: 1, row: -1 },
  { col: -1, row: 0 },
  { col: 1, row: 0 },
  { col: -1, row: 1 },
  { col: 0, row: 1 },
  { col: 1, row: 1 },
];

/** 直線 + 1 マス上下（術師の貫通気味） */
const PATTERN_PIERCE: TilePos[] = [
  { col: 1, row: 0 },
  { col: 2, row: 0 },
  { col: 3, row: 0 },
  { col: 1, row: -1 },
  { col: 1, row: 1 },
];

export const HEROES: HeroDef[] = [
  {
    id: 1001,
    name: "コナン・ドイル",
    class: "supporter",
    rarity: "common",
    attackType: "INT",
    cost: 11,
    phy: 25,
    int: 69,
    phyDef: 15,
    intDef: 40,
    hp: 192,
    agi: 138,
    attackPattern: PATTERN_AROUND,
    imageUrl: `${HERO_IMAGE_BASE}/1001.png`,
  },
  {
    id: 1002,
    name: "甲斐姫",
    class: "vanguard",
    rarity: "common",
    attackType: "PHY",
    cost: 9,
    phy: 79,
    int: 45,
    phyDef: 25,
    intDef: 20,
    hp: 162,
    agi: 118,
    attackPattern: PATTERN_NEAR_3,
    imageUrl: `${HERO_IMAGE_BASE}/1002.png`,
  },
  {
    id: 1003,
    name: "張遼",
    class: "sniper",
    rarity: "common",
    attackType: "PHY",
    cost: 14,
    phy: 67,
    int: 62,
    phyDef: 15,
    intDef: 10,
    hp: 222,
    agi: 93,
    attackPattern: PATTERN_LINE_4,
    imageUrl: `${HERO_IMAGE_BASE}/1003.png`,
  },
  {
    id: 1004,
    name: "シートン",
    class: "caster",
    rarity: "common",
    attackType: "INT",
    cost: 30,
    phy: 66,
    int: 69,
    phyDef: 10,
    intDef: 35,
    hp: 345,
    agi: 46,
    attackPattern: PATTERN_PIERCE,
    imageUrl: `${HERO_IMAGE_BASE}/1004.png`,
  },
  {
    id: 1006,
    name: "ピタゴラス",
    class: "guard",
    rarity: "common",
    attackType: "PHY",
    cost: 25,
    phy: 84,
    int: 84,
    phyDef: 35,
    intDef: 25,
    hp: 216,
    agi: 56,
    attackPattern: PATTERN_NEAR_FAN,
    imageUrl: `${HERO_IMAGE_BASE}/1006.png`,
  },
  {
    id: 1007,
    name: "大長今",
    class: "medic",
    rarity: "common",
    attackType: "INT",
    cost: 24,
    phy: 40,
    int: 115,
    phyDef: 20,
    intDef: 30,
    hp: 261,
    agi: 54,
    attackPattern: PATTERN_AROUND,
    imageUrl: `${HERO_IMAGE_BASE}/1007.png`,
  },
  {
    id: 1008,
    name: "ジョン・L・サリバン",
    class: "defender",
    rarity: "common",
    attackType: "PHY",
    cost: 30,
    phy: 85,
    int: 35,
    phyDef: 60,
    intDef: 30,
    hp: 264,
    agi: 88,
    attackPattern: PATTERN_NEAR_3,
    imageUrl: `${HERO_IMAGE_BASE}/1008.png`,
  },
  {
    id: 1009,
    name: "ヘルクレスオオカブト",
    class: "specialist",
    rarity: "common",
    attackType: "INT",
    cost: 26,
    phy: 72,
    int: 71,
    phyDef: 30,
    intDef: 30,
    hp: 267,
    agi: 64,
    attackPattern: PATTERN_LINE_3,
    imageUrl: `${HERO_IMAGE_BASE}/1009.png`,
  },

  // ─── SPEC-015: Uncommon 10 体 ──────────────────────────────
  // mycryptoheroes/Data/Heroes/heroes.json から rarity Uncommon を 8 職業に
  // 散らした選別。max_level_stats そのまま、phyDef/intDef は職業ベース値。
  {
    id: 2002,
    name: "スパルタクス",
    class: "defender",
    rarity: "uncommon",
    attackType: "PHY",
    cost: 38,
    phy: 59,
    int: 42,
    phyDef: 80,
    intDef: 30,
    hp: 573,
    agi: 42,
    attackPattern: PATTERN_NEAR_3,
    imageUrl: `${HERO_IMAGE_BASE}/2002.png`,
  },
  {
    id: 2013,
    name: "許褚",
    class: "guard",
    rarity: "uncommon",
    attackType: "PHY",
    cost: 30,
    phy: 131,
    int: 51,
    phyDef: 40,
    intDef: 20,
    hp: 285,
    agi: 63,
    attackPattern: PATTERN_NEAR_FAN,
    imageUrl: `${HERO_IMAGE_BASE}/2013.png`,
  },
  {
    id: 2009,
    name: "森蘭丸",
    class: "vanguard",
    rarity: "uncommon",
    attackType: "PHY",
    cost: 14,
    phy: 79,
    int: 98,
    phyDef: 25,
    intDef: 25,
    hp: 168,
    agi: 107,
    attackPattern: PATTERN_NEAR_3,
    imageUrl: `${HERO_IMAGE_BASE}/2009.png`,
  },
  {
    id: 2016,
    name: "アナスタシア",
    class: "specialist",
    rarity: "uncommon",
    attackType: "INT",
    cost: 32,
    phy: 31,
    int: 111,
    phyDef: 25,
    intDef: 35,
    hp: 150,
    agi: 148,
    attackPattern: PATTERN_LINE_3,
    imageUrl: `${HERO_IMAGE_BASE}/2016.png`,
  },
  {
    id: 2003,
    name: "ジャックザリッパー",
    class: "sniper",
    rarity: "uncommon",
    attackType: "PHY",
    cost: 22,
    phy: 108,
    int: 25,
    phyDef: 15,
    intDef: 10,
    hp: 234,
    agi: 123,
    attackPattern: PATTERN_LINE_4,
    imageUrl: `${HERO_IMAGE_BASE}/2003.png`,
  },
  {
    id: 2006,
    name: "アルキメデス",
    class: "caster",
    rarity: "uncommon",
    attackType: "INT",
    cost: 30,
    phy: 75,
    int: 103,
    phyDef: 15,
    intDef: 35,
    hp: 174,
    agi: 98,
    attackPattern: PATTERN_PIERCE,
    imageUrl: `${HERO_IMAGE_BASE}/2006.png`,
  },
  {
    id: 2014,
    name: "徳川慶喜",
    class: "caster",
    rarity: "uncommon",
    attackType: "INT",
    cost: 32,
    phy: 57,
    int: 111,
    phyDef: 12,
    intDef: 35,
    hp: 219,
    agi: 99,
    attackPattern: PATTERN_LINE_4,
    imageUrl: `${HERO_IMAGE_BASE}/2014.png`,
  },
  {
    id: 2011,
    name: "孫子",
    class: "caster",
    rarity: "uncommon",
    attackType: "INT",
    cost: 38,
    phy: 57,
    int: 127,
    phyDef: 10,
    intDef: 40,
    hp: 186,
    agi: 94,
    attackPattern: PATTERN_PIERCE,
    imageUrl: `${HERO_IMAGE_BASE}/2011.png`,
  },
  {
    id: 2007,
    name: "サンタクロース",
    class: "medic",
    rarity: "uncommon",
    attackType: "INT",
    cost: 28,
    phy: 43,
    int: 96,
    phyDef: 20,
    intDef: 35,
    hp: 255,
    agi: 116,
    attackPattern: PATTERN_AROUND,
    imageUrl: `${HERO_IMAGE_BASE}/2007.png`,
  },
  {
    id: 2010,
    name: "カフカ",
    class: "supporter",
    rarity: "uncommon",
    attackType: "INT",
    cost: 18,
    phy: 46,
    int: 68,
    phyDef: 15,
    intDef: 45,
    hp: 234,
    agi: 148,
    attackPattern: PATTERN_AROUND,
    imageUrl: `${HERO_IMAGE_BASE}/2010.png`,
  },

  // ─── SPEC-021: Rare 8 体（MCH 公式 DB の ID と nameJa / passive を採用）
  // MCH 上の役割（passive 内容 + 統計プロファイル）から TD クラスへマップ。
  // ステータスは TD バランス用に既存値を維持。
  {
    id: 3001,
    name: "ETHEREMON-RED",
    class: "defender",
    rarity: "rare",
    attackType: "PHY",
    cost: 36,
    phy: 73,
    int: 102,
    phyDef: 85,
    intDef: 40,
    hp: 612,
    agi: 58,
    attackPattern: PATTERN_NEAR_3,
    imageUrl: `${HERO_IMAGE_BASE}/3001.png`,
  },
  {
    id: 3002,
    name: "ダルタニャン",
    class: "vanguard",
    rarity: "rare",
    attackType: "PHY",
    cost: 16,
    phy: 134,
    int: 58,
    phyDef: 30,
    intDef: 25,
    hp: 232,
    agi: 132,
    attackPattern: PATTERN_NEAR_3,
    imageUrl: `${HERO_IMAGE_BASE}/3002.png`,
  },
  {
    id: 3003,
    name: "平賀源内",
    class: "supporter",
    rarity: "rare",
    attackType: "INT",
    cost: 22,
    phy: 56,
    int: 121,
    phyDef: 20,
    intDef: 50,
    hp: 246,
    agi: 134,
    attackPattern: PATTERN_AROUND,
    imageUrl: `${HERO_IMAGE_BASE}/3003.png`,
  },
  {
    id: 3004,
    name: "マタ・ハリ",
    class: "specialist",
    rarity: "rare",
    attackType: "INT",
    cost: 28,
    phy: 64,
    int: 119,
    phyDef: 35,
    intDef: 35,
    hp: 274,
    agi: 138,
    attackPattern: PATTERN_LINE_3,
    imageUrl: `${HERO_IMAGE_BASE}/3004.png`,
  },
  {
    id: 3005,
    name: "ETHEREMON-BLUE",
    class: "caster",
    rarity: "rare",
    attackType: "INT",
    cost: 34,
    phy: 88,
    int: 132,
    phyDef: 18,
    intDef: 42,
    hp: 256,
    agi: 102,
    attackPattern: PATTERN_PIERCE,
    imageUrl: `${HERO_IMAGE_BASE}/3005.png`,
  },
  {
    id: 3006,
    name: "ETHEREMON-GREEN",
    class: "medic",
    rarity: "rare",
    attackType: "INT",
    cost: 30,
    phy: 48,
    int: 124,
    phyDef: 25,
    intDef: 38,
    hp: 296,
    agi: 124,
    attackPattern: PATTERN_AROUND,
    imageUrl: `${HERO_IMAGE_BASE}/3006.png`,
  },
  {
    id: 3007,
    name: "皇帝ネロ",
    class: "guard",
    rarity: "rare",
    attackType: "PHY",
    cost: 32,
    phy: 142,
    int: 71,
    phyDef: 45,
    intDef: 30,
    hp: 348,
    agi: 96,
    attackPattern: PATTERN_NEAR_FAN,
    imageUrl: `${HERO_IMAGE_BASE}/3007.png`,
  },
  {
    id: 3008,
    name: "ノストラダムス",
    class: "sniper",
    rarity: "rare",
    attackType: "INT",
    cost: 24,
    phy: 47,
    int: 138,
    phyDef: 22,
    intDef: 18,
    hp: 268,
    agi: 142,
    attackPattern: PATTERN_LINE_4,
    imageUrl: `${HERO_IMAGE_BASE}/3008.png`,
  },

  // ─── SPEC-021: SuperRare 8 体（MCH "Epic" tier。ID と nameJa / passive を採用）
  // SR は MCH 上は "epic" だが、コード側のレアリティ階層では superRare に対応。
  {
    id: 4001,
    name: "張飛",
    class: "defender",
    rarity: "superRare",
    attackType: "PHY",
    cost: 40,
    phy: 162,
    int: 88,
    phyDef: 92,
    intDef: 48,
    hp: 728,
    agi: 64,
    attackPattern: PATTERN_NEAR_3,
    imageUrl: `${HERO_IMAGE_BASE}/4001.png`,
  },
  {
    id: 4002,
    name: "ナイチンゲール",
    class: "medic",
    rarity: "superRare",
    attackType: "INT",
    cost: 34,
    phy: 56,
    int: 178,
    phyDef: 28,
    intDef: 56,
    hp: 348,
    agi: 138,
    attackPattern: PATTERN_AROUND,
    imageUrl: `${HERO_IMAGE_BASE}/4002.png`,
  },
  {
    id: 4003,
    name: "ベートーヴェン",
    class: "caster",
    rarity: "superRare",
    attackType: "INT",
    cost: 42,
    phy: 102,
    int: 178,
    phyDef: 18,
    intDef: 48,
    hp: 312,
    agi: 116,
    attackPattern: PATTERN_PIERCE,
    imageUrl: `${HERO_IMAGE_BASE}/4003.png`,
  },
  {
    id: 4004,
    name: "佐々木小次郎",
    class: "vanguard",
    rarity: "superRare",
    attackType: "PHY",
    cost: 22,
    phy: 174,
    int: 86,
    phyDef: 32,
    intDef: 28,
    hp: 268,
    agi: 168,
    attackPattern: PATTERN_NEAR_3,
    imageUrl: `${HERO_IMAGE_BASE}/4004.png`,
  },
  {
    id: 4005,
    name: "勝海舟",
    class: "guard",
    rarity: "superRare",
    attackType: "PHY",
    cost: 36,
    phy: 162,
    int: 102,
    phyDef: 52,
    intDef: 36,
    hp: 412,
    agi: 108,
    attackPattern: PATTERN_NEAR_FAN,
    imageUrl: `${HERO_IMAGE_BASE}/4005.png`,
  },
  {
    id: 4006,
    name: "ビリー・ザ・キッド",
    class: "sniper",
    rarity: "superRare",
    attackType: "INT",
    cost: 28,
    phy: 64,
    int: 168,
    phyDef: 22,
    intDef: 32,
    hp: 286,
    agi: 154,
    attackPattern: PATTERN_LINE_4,
    imageUrl: `${HERO_IMAGE_BASE}/4006.png`,
  },
  {
    id: 4007,
    name: "トーマス・エジソン",
    class: "specialist",
    rarity: "superRare",
    attackType: "INT",
    cost: 38,
    phy: 64,
    int: 168,
    phyDef: 30,
    intDef: 42,
    hp: 312,
    agi: 132,
    attackPattern: PATTERN_LINE_3,
    imageUrl: `${HERO_IMAGE_BASE}/4007.png`,
  },
  {
    id: 4008,
    name: "マルコ・ポーロ",
    class: "supporter",
    rarity: "superRare",
    attackType: "INT",
    cost: 30,
    phy: 64,
    int: 162,
    phyDef: 22,
    intDef: 56,
    hp: 296,
    agi: 138,
    attackPattern: PATTERN_AROUND,
    imageUrl: `${HERO_IMAGE_BASE}/4008.png`,
  },

  // ─── SPEC-021: Legendary 4 体（MCH 公式 DB の ID と nameJa / passive を採用）
  {
    id: 5001,
    name: "織田信長",
    class: "supporter",
    rarity: "legendary",
    attackType: "PHY",
    cost: 36,
    phy: 232,
    int: 86,
    phyDef: 28,
    intDef: 64,
    hp: 384,
    agi: 162,
    attackPattern: PATTERN_AROUND,
    imageUrl: `${HERO_IMAGE_BASE}/5001.png`,
  },
  {
    id: 5002,
    name: "ナポレオン・ボナパルト",
    class: "caster",
    rarity: "legendary",
    attackType: "INT",
    cost: 44,
    phy: 96,
    int: 232,
    phyDef: 22,
    intDef: 58,
    hp: 348,
    agi: 168,
    attackPattern: PATTERN_PIERCE,
    imageUrl: `${HERO_IMAGE_BASE}/5002.png`,
  },
  {
    id: 5003,
    name: "曹操",
    class: "defender",
    rarity: "legendary",
    attackType: "PHY",
    cost: 48,
    phy: 218,
    int: 112,
    phyDef: 110,
    intDef: 56,
    hp: 892,
    agi: 72,
    attackPattern: PATTERN_NEAR_3,
    imageUrl: `${HERO_IMAGE_BASE}/5003.png`,
  },
  {
    id: 5004,
    name: "ジョージ・ワシントン",
    class: "vanguard",
    rarity: "legendary",
    attackType: "PHY",
    cost: 32,
    phy: 158,
    int: 158,
    phyDef: 38,
    intDef: 32,
    hp: 332,
    agi: 224,
    attackPattern: PATTERN_NEAR_3,
    imageUrl: `${HERO_IMAGE_BASE}/5004.png`,
  },
];

export function findHero(id: number): HeroDef | undefined {
  return HEROES.find((h) => h.id === id);
}
